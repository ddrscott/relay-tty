...
pty-host waits 100ms for a `RESUME` message. If none arrives (e.g., CLI clients), it sends the full buffer. A `SPARKLINE_REQUEST` sent as the first message is answered directly without a replay.
...
| Field | Type | Meaning |
|-------|------|---------|
| `agentState` | `"working" \| "blocked" \| "done" \| "idle" \| "unknown"` | Heuristic state of the foreground agent, recomputed once a second from `foregroundProcess`, `bps1`, and the last 4KB of output with escapes stripped. `idle` when the shell prompt is in the foreground, `unknown` for processes that are not a recognized agent. `blocked` means a permission or confirmation prompt is on screen. `done` means a known agent settled while no client was connected; it clears to `idle` on the next connect. |
