...
| `RELAY_VNC_PORT` | No | Loopback port of the VNC server behind `/desktop` (default: `5900`). Set for Linux displays on `5901`+ |
...
|----------|-------------|
| `RELAY_SESSION_ID` | The 8-character hex session ID |
| `RELAY_ORIG_COMMAND` | The command that was passed to `relay` |
| `RELAY_ORIG_ARGS` | JSON array of the original command arguments |
