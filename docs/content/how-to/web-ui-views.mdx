...
- **File paths** — clickable file paths (and `file://` links) open in the in-app syntax-highlighted viewer, resolved on the machine running the session — not your browser. Line and column numbers (`foo.ts:42:10`, `file://…/foo.ts#42:10`) jump to that location.
