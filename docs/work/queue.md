...
- [-] Perf: pty-host must answer SPARKLINE_REQUEST as a first frame instead of sending a phantom full replay — removes 12×2s stalls that block xterm module loads ([details](sparkline-first-frame.md))
- [ ] Perf: one shared /ws/events socket per page + batched metrics/sidebar updates to stop periodic main-thread stalls while typing ([details](shared-events-socket.md))
