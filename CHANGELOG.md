...
### Added
- `relay` with no arguments opens the TUI, which is now a real terminal client with a tmux-style prefix key. Attach with Enter, then `Ctrl+B n`/`p` switch between running sessions, `1`-`9` jump, `c` opens a new shell in the current session's directory, `,` renames, `x` interrupts, `k` clears scrollback, `a` opens an actions menu, `s` or `Esc` returns to the picker, `d` detaches, `?` lists the keys, and `Ctrl+B Ctrl+B` sends the key literally. The prefix is configurable with `prefix = C-a` in `~/.config/relay-tty/relayrc`, which any `relay` command creates with the defaults the first time it runs without one. The picker adds a colored agent-state column with blocked sessions first, number keys, `c` and `,`, live updates from the session directory instead of a 2s poll, and `--host` to browse and attach to sessions on another machine through its HTTP and WebSocket API
- Agent state. pty-host classifies each session's foreground agent once a second as `working`, `blocked`, `done`, `idle`, or `unknown` (Claude Code, Codex, opencode, aider, gemini, copilot, goose, amp, grok, and Claude Code's version-numbered binary are recognized) from the process name, the last 4KB of output with escapes stripped, and throughput. `agentState` and `agentStateChangedAt` ride in the session JSON and `SESSION_UPDATE`; the web sidebar and activity cards show a `BLOCKED`/`WORKING`/`DONE` chip and the "active" sort puts blocked sessions first; a new "Agent blocked" push notification trigger fires when an agent stops to ask for permission
- CLI commands for scripts and other agents: `relay send <id> [text] [--enter]`, `relay rename <id> <title> [--unpin]`, `relay kill <id> [--signal]`, `relay wait <id> --state <blocked|working|done|idle|unknown|exited> [--timeout]`, `relay events` (JSON lines), `relay list --json --watch`, and `relay info <id>`. All accept `--host`. Titles set with `rename` are pinned so a shell or editor's OSC title no longer overwrites them, in the CLI, the TUI and the browser tab
- Protocol: `SET_TITLE` (0x24) pins a title, `SIGNAL` (0x25) delivers a signal to the foreground process group, and `OBSERVE` (0x26) as a first frame subscribes to live frames with no replay and without counting as an attached viewer. Server monitors now connect as observers, so a session can reach `done` while only the server is watching

### Changed
- One client core. `shared/client/` holds the session protocol client (`SessionStream` over a Unix-socket or WebSocket transport) and the session directory (disk or remote). The browser terminal hooks, `relay attach`, the TUI preview, the server's per-session monitors and every new CLI command are consumers of it, replacing four separate implementations of the RESUME/SYNC handshake and frame parsing. A conformance test runs the client against a real pty-host over both transports
