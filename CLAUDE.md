...
## TUI (`cli/tui/`)
`relay` with no arguments opens the TUI. `keys.ts` is the pure prefix-key machine (default Ctrl+B, `prefix = C-x` in `~/.config/relay-tty/relayrc` via `cli/rc.ts`); `index.ts` runs the picker and the attach loop (a fresh `SessionStream` per switch, tail-limited replay repaints the screen); `status.ts` owns the last row (status line, line prompt, menu). The picker mirrors the web sidebar: `tree.ts` builds its rows from `groupByCwd`, `sortSessions`, and `filterByStatus` in `app/lib/session-groups.ts`, which both clients share, so sort and filter behavior changes there and not in either client. `tree.ts` also owns the 1-9 numbering and the n/p cycle, so the picker and `Ctrl+B 1-9` cannot disagree. View state persists in `~/.relay-tty/tui.json` (`prefs.ts`). Anything that repaints the picker must check `state.prompting`, because a prompt or menu owns the last row.
